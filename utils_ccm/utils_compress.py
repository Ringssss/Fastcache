...
def get_underlying_model(model):
    # 支持多种模型结构的底层transformer模型查找
    # LLaVA系列: language_model, llava_model, llama_model, decoder
    # MiniCPM系列: llm -> model (嵌套结构)
    ...
    for attr in possible_model_attrs:
        if hasattr(model, attr):
            ...
            if hasattr(potential_model, 'layers'):
                ...
                print(f"🔍 Found underlying model via '{attr}': {type(underlying_model)}")
                break
            # 检查是否有transformer属性
            elif hasattr(potential_model, 'transformer'):
                underlying_model = potential_model
                print(f"🔍 Found underlying model via '{attr}.transformer': {type(underlying_model)}")
                break
            # 特殊处理：检查是否有model属性（如MiniCPM的llm.model结构）
            elif hasattr(potential_model, 'model'):
                inner_model = getattr(potential_model, 'model')
                if hasattr(inner_model, 'layers'):
                    underlying_model = inner_model
                    print(f"🔍 Found underlying model via '{attr}.model': {type(underlying_model)}")
                    break
    
    if underlying_model is None:
        # 提供更详细的错误信息
        available_attrs = [attr for attr in possible_model_attrs if hasattr(model, attr)]
        print(f"❌ Could not find underlying transformer model in {type(model)}")
