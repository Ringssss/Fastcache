    ...
    def _collect_gpu_stats(self):
        ...
        while self.is_monitoring:
            try:
                ...
                self.gpu_stats['batch_size'].append(self.batch_size)
...
class DynamicScheduler:
    ...
    # def start_memory_monitor(self, interval=0.1):
    #     """启动内存监控线程"""
    #     def monitor_memory():
    ...
    #                 # 'cached': torch.cuda.memory_cached() / (1024 * 1024)         # MB
    #             }
    ...
    def adaptive_batch_size(self, queue_type, current_performance, gpu_stats=None):
        # print("gpu_stats", gpu_stats['batch_size'][0] if gpu_stats  is not None else 0)
        if gpu_stats is not None:
            batch_size = gpu_stats['batch_size'][0]
            base_memory_used = gpu_stats['memory_used'][0]
            peak_memory_used = max(gpu_stats['memory_used'])
            memory_used = peak_memory_used - base_memory_used
            print("memory_used", memory_used, batch_size, memory_used/batch_size)
        ...
        if queue_type == 'prefill':
            ...
            # # current_size = sum(self.decoding_lens_list)
            ...
            i_count = self.binary_search_last_le(cumsum_list, self.std_batch_threshold_decoding)+1
    ...
    def select_batch(self, last_prefill_time, max_wait_time, gpu_stats=None):
        """选择下一批处理的请求"""
        ...
        batch_size = self.adaptive_batch_size(self.current_mode, current_perf, gpu_stats)
