...
from nanovllm.engine.auto_compress_controller import AutoCompressionController
...
class LlavaLLMEngine:
    ...
    def __init__(
        ...
        auto_tune_interval: int = 4,
        auto_dual_batch: bool = True,
        auto_dual_interval: int = 4,
        auto_min_seq_len: int = 256,
        auto_min_output_len: int = 64,
        auto_min_speedup: float = 1.05,
        auto_force_compress: bool = False,
        **kwargs
    ):
        """
        初始化LLaVA推理引擎
        ...
        Args:
            ...
            compression_pipeline: 压缩流水线策略 (auto/prefill/decode)
            compression_streams: Prefill流水线压缩stream数量
            greenctx_enabled: 是否启用greenctx stream分配
            greenctx_compress_ratio: 压缩stream SM占比
            greenctx_main_ratio: 主stream SM占比
            greenctx_main_stream: 是否让主流使用greenctx stream
            decode_layers_per_step: Decode流水线每步压缩层数
            auto_tune: 自动调度pipeline/overlap/SM分配
            auto_tune_interval: 自动调度更新间隔
            auto_dual_batch: 自动调度prefill/decode批次比例
            auto_dual_interval: dual-batch调度更新间隔
            auto_min_seq_len: 自动压缩最小prefill长度
            auto_min_output_len: 自动压缩最小输出长度
            auto_min_speedup: 自动压缩最小预测加速
            auto_force_compress: 忽略阈值强制压缩
            **kwargs: 其他配置参数
        """
        config = Config(model)
        ...
        self.model_runner = LlavaModelRunner(
            ...
            greenctx_main_stream=greenctx_main_stream,
            decode_layers_per_step=decode_layers_per_step,
        )
        ...
        self.compression_pipeline = self.model_runner.compression_pipeline
        self.auto_tune = auto_tune
        self.auto_tune_interval = max(1, auto_tune_interval)
        self.auto_dual_batch = auto_dual_batch
        self.auto_dual_interval = max(1, auto_dual_interval)
        self._auto_controller = None
        if self.auto_tune and self.enable_compression:
            self._auto_controller = AutoCompressionController(
                compression_backend=self.compression_backend,
                kvpress_method=self.kvpress_method,
                compression_factor=self.compression_factor,
                num_layers=self.model_runner.num_hidden_layers,
                num_kv_heads=self.model_runner.num_key_value_heads,
                head_dim=self.model_runner.head_dim,
                greenctx_enabled=self.model_runner.greenctx_enabled,
                decision_interval=self.auto_tune_interval,
                min_seq_len=auto_min_seq_len,
                min_output_len=auto_min_output_len,
                min_speedup=auto_min_speedup,
                force_compress=auto_force_compress,
                dual_batch_enabled=auto_dual_batch,
                dual_batch_interval=self.auto_dual_interval,
            )

        self._seq_pipeline = {}
        self._pending_compression_events: dict[int, PendingCompression] = {}
        self._decode_overlap_start: dict[int, float] = {}
        self._auto_step = 0
        ...
        print(f"  - 压缩流水线: {self.compression_pipeline}")
        print(f"  - 压缩因子: {compression_factor}")
        if self.auto_tune:
            print(f"  - 自动调度: 开启 (interval={self.auto_tune_interval})")
    ...
    def step(self, apply_compression: bool = False) -> Tuple[list, int]:
        ...
        auto_enabled = (
            ...
            and apply_compression
            and self.enable_compression
        )
        prefer_decode = False
        if auto_enabled and self.auto_dual_batch:
            choice = self._auto_controller.decide_batch_type(
                waiting=len(self.scheduler.waiting),
                running=len(self.scheduler.running),
                pending_compressions=len(self._pending_compression_events),
            )
            prefer_decode = choice == "decode"
        seqs, is_prefill = self.scheduler.schedule(prefer_decode=prefer_decode)
        ...
        pipeline = self.compression_pipeline
        async_compress = self.async_compression
        apply_prefill_compression = apply_compression and is_prefill and self.enable_compression

        if is_prefill:
            if auto_enabled:
                expected_output_len = max(
                    1,
                    int(sum(seq.max_tokens for seq in seqs) / max(1, len(seqs))),
                )
                seq_len = max(len(seq) for seq in seqs) if seqs else 0
                decision = self._auto_controller.decide(
                    batch_size=len(seqs),
                    seq_len=seq_len,
                ...
                async_compress = decision.async_compress
                if pipeline == "none":
                    apply_prefill_compression = False
                else:
                    apply_prefill_compression = True
                self.model_runner.update_streams(
                    use_greenctx=decision.greenctx_enabled,
                    sm_compress_ratio=decision.sm_compress_ratio,
                    sm_main_ratio=decision.sm_main_ratio,
                    use_greenctx_main=self.model_runner.greenctx_main_stream,
                    compression_streams=decision.compression_streams,
                )
                if pipeline == "decode":
                    self.model_runner.update_decode_overlap_engine(
                        decision.decode_layers_per_step
                    )

            pipeline_tag = pipeline if apply_prefill_compression else "none"
            for seq in seqs:
                self._seq_pipeline[seq.seq_id] = pipeline_tag

        # Prefill-bound pipeline: wait before decode
        if not is_prefill and self.enable_compression:
            prefill_seqs = [
                seq for seq in seqs if seq.seq_id in self._pending_compression_events
            ]
            if prefill_seqs:
                self._wait_pending_compressions(prefill_seqs)
                self._free_compressed_blocks(prefill_seqs)

        # 运行模型
        apply_in_run = (
            apply_prefill_compression
            and pipeline == "prefill"
            and not async_compress
        )

        use_timing = False
        if auto_enabled:
            self._auto_step += 1
            use_timing = (self._auto_step % self.auto_tune_interval) == 0
        if use_timing:
            start_event = torch.cuda.Event(enable_timing=True)
            end_event = torch.cuda.Event(enable_timing=True)
            main_stream = getattr(self.model_runner, "_main_stream", None)
            stream_ctx = torch.cuda.stream(main_stream) if main_stream is not None else nullcontext()
            with stream_ctx:
                start_event.record()
                token_ids = self.model_runner.run(
                    ...
                    is_prefill,
                    pixel_values=pixel_values,
                    apply_compression=apply_in_run,
                )
                end_event.record()
        ...
        if use_timing and elapsed_ms is not None:
            if is_prefill:
                prefill_tokens = sum(len(seq) - seq.num_cached_tokens for seq in seqs)
                compress_ms = None
                if apply_in_run and self.model_runner._last_compress_time_ms is not None:
                    if self.model_runner._last_compress_tokens > 0:
                        compress_ms = self.model_runner._last_compress_time_ms
                prefill_ms = elapsed_ms
                if compress_ms is not None:
                    prefill_ms = max(0.0, prefill_ms - compress_ms)
                    self._auto_controller.update_compress(compress_ms)
                self._auto_controller.update_prefill(prefill_ms, prefill_tokens)
            else:
                self._auto_controller.update_decode(elapsed_ms, len(seqs))

        if apply_prefill_compression:
            if pipeline == "prefill":
                if async_compress:
                ...
                self.model_runner.start_decode_overlap_compression(seqs)
                for seq in seqs:
                    self._decode_overlap_start[seq.seq_id] = perf_counter()

        # Decode-bound pipeline: progress compression after decode
        if not is_prefill and self.enable_compression:
            decode_seqs = [
                seq for seq in seqs if self._seq_pipeline.get(seq.seq_id) == "decode"
            ]
            if decode_seqs:
                completed = self.model_runner.step_decode_overlap_compression(decode_seqs)
                if completed:
                    self._free_compressed_blocks(completed)
                    if auto_enabled:
                        elapsed_list = []
                        now = perf_counter()
                        for seq in completed:
                            start = self._decode_overlap_start.pop(seq.seq_id, None)
                            if start is not None:
                                elapsed_list.append((now - start) * 1000)
                        if elapsed_list:
                            self._auto_controller.update_compress(
        ...
        if outputs:
            for seq_id, _ in outputs:
                self._seq_pipeline.pop(seq_id, None)
                self._pending_compression_events.pop(seq_id, None)
                self._decode_overlap_start.pop(seq_id, None)
    ...
    def _start_pipeline_compression(self, seqs):
        ...
        """
        # 启动异步压缩并保存事件
        compress_stream = self.model_runner.get_next_compress_stream()
        event_pair = self.model_runner.start_background_compression(seqs, compress_stream=compress_stream)
        if event_pair is None:
            return
        start_event, end_event = event_pair
        tokens = sum(len(seq) for seq in seqs)
        pending = PendingCompression(start_event=start_event, end_event=end_event, tokens=tokens)
        for seq in seqs:
            self._pending_compression_events[seq.seq_id] = pending
    ...
    def _wait_pending_compressions(self, seqs):
        ...
        """
        for seq in seqs:
            pending = self._pending_compression_events.get(seq.seq_id)
            if pending is None:
                continue
            pending.end_event.synchronize()
            if self._auto_controller is not None and not pending.reported:
                pending.reported = True
                elapsed_ms = pending.start_event.elapsed_time(pending.end_event)
                self._auto_controller.update_compress(elapsed_ms)
