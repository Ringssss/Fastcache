...
class StreamPriorityManager:
    ...
    def __init__(
        ...
        compress_stream: torch.cuda.Stream = None,
        decode_priority: int = -1,
        compress_priority: int = 0,
    ):
...
class ZeroOverheadCompressEngine:
    ...
    def __init__(
        ...
        # Stream管理
        self.stream_manager = StreamPriorityManager(
            self.device,
    ...
    def step_compression(self, seq_id: int) -> bool:
        ...
        with torch.no_grad():
            if hasattr(self.compressor, "compress_layers"):
                compressed_chunk = self.compressor.compress_layers(
                    state['kv_cache'],
                ...
                for offset, compressed in enumerate(compressed_chunk):
                    state['compressed'][start_layer + offset] = compressed
            else:
                with torch.cuda.stream(compress_stream):
                    for layer_idx in range(start_layer, end_layer):
