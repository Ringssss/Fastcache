#!/usr/bin/env python3
"""
Qwen3 + kvpress 在 nano-vllm 中的基准测试
==========================================

测试kvpress压缩方法在Qwen3模型上通过nano-vllm框架的性能。
由于没有针对Qwen3训练的MLP压缩器，这里只测试kvpress方法。
...
def print_results_table(results: List[BenchResult]):
    """打印结果表格"""
    print("\n" + "="*100)
    print(" QWEN3 + KVPRESS BENCHMARK RESULTS")
    print("="*100)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Qwen3 + kvpress in nano-vllm Benchmark')
    ...
    print("#" * 80)
    print(" Qwen3 + kvpress in nano-vllm Benchmark")
    print("#" * 80)
    print(f"\nModel: {args.model}")
    ...
    if args.output:
        ...
        save_results(results, f"qwen3_kvpress_nanovllm_{timestamp}.json")
