#!/usr/bin/env python3
"""
kvpress在nano-vllm中的基准测试
================================

测试kvpress压缩方法在nano-vllm框架中的性能:
- 对比无压缩 vs kvpress压缩
- 对比不同kvpress方法 (streaming_llm, knorm, random)
- 对比不同batch size
- 测试异步压缩的效果
...
class BenchResult:
    """基准测试结果"""
    model_name: str
    backend: str  # 'none', 'mlp', 'kvpress'
    method: str   # kvpress方法名或'mlp'
...
def run_full_benchmark(
    ...
    for batch_size in batch_sizes:
        ...
        # 4. MLP压缩 (如果有compressor_path)
        if compressor_path and os.path.exists(compressor_path):
            print(f"\n[4] MLP compression")
            try:
                result = run_benchmark_mlp(
                    model_path, compressor_path, batch_size, seq_len, output_len,
                    compression_factor=compression_factor,
                    async_compress=False,
                )
                results.append(result)
...
def print_results_table(results: List[BenchResult]):
    ...
    for r in results:
        ...
        key = f"{r.backend}_{r.method}" + ("_async" if r.async_compress else "")
        by_batch[r.batch_size][key] = r

    # 获取所有方法
    all_methods = sorted(set(
        f"{r.backend}_{r.method}" + ("_async" if r.async_compress else "")
    ...
    for method in all_methods:
        short = method.replace('kvpress_', 'kv:').replace('none_none', 'baseline')
        header += f" {short:>18} |"
    print(header)
    print("-"*len(header))
    ...
    for batch in sorted(by_batch.keys()):
        ...
        baseline = methods.get('none_none')
        for method in all_methods:
            r = methods.get(method)
            if r:
                tp = r.throughput_tok_s
                if method != 'none_none' and baseline:
                    speedup = tp / baseline.throughput_tok_s
                    row += f" {tp:>10.0f} ({speedup:.2f}x) |"
                else:
                    row += f" {tp:>18.0f} |"
            else:
                row += f" {'N/A':>18} |"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='kvpress in nano-vllm Benchmark')
    ...
    parser.add_argument('--compressor', default=str(CKPT_DIR / "llava_mlp.pth"),
                        help='Compressor checkpoint path (optional, for MLP comparison)')
    parser.add_argument('--batch-sizes', type=int, nargs='+',
                        default=[1, 8, 32, 64, 128],
                        help='Batch sizes to test')
    parser.add_argument('--kvpress-methods', type=str, nargs='+',
                        default=['streaming_llm', 'knorm', 'random'],
                        help='kvpress methods to test')
    ...
    print("#" * 80)
    print(" kvpress in nano-vllm Benchmark")
    print("#" * 80)
    print(f"\nModel: {args.model}")
    ...
    if args.output:
        ...
        save_results(results, f"kvpress_nanovllm_bench_{timestamp}.json")
