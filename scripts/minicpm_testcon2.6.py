...
import nvtx
from transformers import AutoTokenizer, AutoModel, AutoProcessor,Cache, DynamicCache
import argparse
import yaml


from utils_ccm.module_ccm_v11 import KVCacheHybridCompressor, CustomImageTextMinicpmDataset
...
class CustomImageTextMinicpmDatasetForRun(Dataset):
    """自定义数据集类"""

    def __init__(self, data, model_runner, processor, max_length=128):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        encoding = self.processor(
                    [self.processor.tokenizer.apply_chat_template(image_text, tokenize=False, add_generation_prompt=True)],
                    [image],
                    padding='max_length',
                    truncation=True,
                    max_length=self.max_length,
        ...
        #     "tgt_sizes": encoding['tgt_sizes'],
        #     'type': torch.Tensor([1]),
        # }
        model_inputs = {
            "input_ids": encoding["input_ids"].squeeze(0),
            "attention_mask": encoding["attention_mask"].squeeze(0),
            "image_bound": encoding["image_bound"][0],
            "pixel_values": encoding['pixel_values'][0],
            # "pixel_values": [[p[0].squeeze(0)] for p in encoding["pixel_values"]],
            "tgt_sizes": encoding['tgt_sizes'][0],
            'type': torch.Tensor([1]),
        }
        ...
        # encoding_result = {
        #     'input_ids': model_inputs['input_ids'].squeeze(0).to("cpu"),
        #     'attention_mask': model_inputs['attention_mask'].squeeze(0).to("cpu"),
        #     # 'inputs_embeds': model_inputs["inputs_embeds"].squeeze(0).to("cpu"),
        #     'type': torch.Tensor([1])
        # }
        # print("encoding_result['input_ids']", model_inputs['input_ids'].shape)
        # print("encoding_result['attention_mask']", model_inputs['attention_mask'].shape)
        # print("encoding_result['inputs_embeds']", encoding_result['inputs_embeds'].shape)
        # print("encoding_result['type']", encoding_result['type'].shape)
...
class ImprovedConcurrentTester:
    ...
    def __init__(self, model, processor, compressor, max_new_tokens, num_samples, std_batch_threshold_prefill, 
                 compress_batch, std_batch_threshold_decoding,
        ...
        self.OPERATIONAL_INSTENSITY = 135
        # self.check_gpu_flag = True
        self.GPU_monitor = GPUMonitor(interval=0.02)
        self.kvcache_pool = KVCachePool(device=device)
        self.dynamic_batching = self.set_dynamic_batching(
            std_batch_threshold_prefill=std_batch_threshold_prefill,
    ...
    def set_dynamic_batching(self, std_batch_threshold_prefill, compress_batch, std_batch_threshold_decoding, comp_mode):
        if comp_mode == 'ccm':
            self.std_batch_threshold_decoding = self.OPERATIONAL_INSTENSITY
            self.compress_batch = self.std_batch_threshold_decoding//3
            self.std_batch_threshold_prefill = self.compress_batch//3
        else:
            self.std_batch_threshold_prefill = std_batch_threshold_prefill
            self.compress_batch = compress_batch
    ...
    def get_comp_len(self, input_ids, attention_mask, kv_len):
        ...
        for i_idx in range(batch_size):
            comp_len[i_idx][0] = int(padding_over_pos[i_idx])
            comp_len[i_idx][1] = 0
            comp_len[i_idx][2] = kv_len - comp_len[i_idx][1] - comp_len[i_idx][0]
            # print(f"comp_len: {comp_len[i_idx]}")
        return comp_len
    ...
    def _exec_compress(self, comp_mode, inputs, request_ids_list):
        ...
        if comp_mode == 'ccm':
            ...
            (
                inputs_embeds,
                vision_hidden_states,
            ) = self.model.get_vllm_embedding(inputs)
            ...
            print(f"comp_mode {comp_mode} press {press}")
            ...
            # TODO normal exec_compression_methods
            comp_past_key_values = exec_compression_methods(self.model.llm,
                                                            underlying_model, inputs_forward, press,
                                                            comp_mode)
                ...
                if key in d:
                    ...
                    if not isinstance(tensor, torch.Tensor):
                        if isinstance(tensor, list):
                            ...
                            raise ValueError(f"张量 {key} 不是torch.Tensor，实际类型为 {type(tensor)}")
    ...
    def run_concurrent_test(self, dataloader, max_workers=4, num_samples=10, max_serve_batch_size=8,
        ...
        middleware = ServiceMiddleware(max_workers, max_serve_batch_size, min_batch_threshold,
                                       std_batch_threshold_decoding, max_wait_time, comp_mode)
        ...
        with nvtx.annotate("prefill_processing", color="blue"):
            with torch.cuda.stream(self.cuda_stream):
                ...
                inputs = {
                    ...
                    # TODO 暂时性.squeeze(0)
                    'pixel_values': [[p[0].squeeze(0).to(self.device)] for p in batch['pixel_values']],
                ...
                # with torch.no_grad(), autocast(dtype=torch.float):
                    ...
                    if use_compression:
                        ...
                        past_key_values, compression_time, compression_ratio, prefill_time = \
                            self._exec_compress(comp_mode, inputs, request_ids_list)
                        ...
                        outputs = self.model.llm(
                            inputs_embeds=inputs_embeds[:,:-1],
                            attention_mask=inputs["attention_mask"][:,:-1],
                            # pixel_values=inputs_forward["pixel_values"],
                            use_cache=True,
                            return_dict=True
                        )
                        # torch.cuda.synchronize()
    ...
    def _process_decoding(self, prefill_result, use_compression, comp_mode):
        ...
        with torch.cuda.stream(self.cuda_stream):
            ...
            gen_outputs = self.model.llm.generate(
                ...
                eos_token_id=self.model.terminators_list,
                do_sample=True,
                temperature=0.7,
                use_cache=True,
...
def exec_testit(test_batching_list, i_setting, item_setting, i_req_sec,
                ...
                gpu_info_path=None, log_save=True
                ):
    ...
    args_ImprovedConcurrentTester = [
        model, processor, compressor, max_new_tokens, num_samples, std_batch_threshold_prefill, 
        compress_batch, std_batch_threshold_decoding,
    ...
    std_batch_threshold_prefill = tester.std_batch_threshold_prefill
    max_serve_batch_size = std_batch_threshold_prefill  # 测试批次数
    min_batch_threshold = std_batch_threshold_prefill
    compress_batch = tester.compress_batch
    std_batch_threshold_decoding = tester.std_batch_threshold_decoding
...
#     folder_path = 'results_0731_log/results_shan1/'
...
#     # 加载模型和处理器
#     print("Loading MiniCPM model and processor...")
#     model = AutoModel.from_pretrained(model_path, torch_dtype=torch_dtype, trust_remote_code=True).to(device)
#     processor = AutoProcessor.from_pretrained(model_path, torch_dtype=torch_dtype, trust_remote_code=True)
...
#     print("processor.image_processor.patch_size", processor.image_processor.patch_size)

#     # 初始化压缩器
#     print("Initializing compressor...")
#     compressor = KVCacheHybridCompressor(
#         src_config=model.config,
#         compression_factor=int(compression_ratio_list[0]),
#         min_seq_len=2,
#     ).to(device, dtype=torch_dtype)
#     terminators_list = [processor.tokenizer.convert_tokens_to_ids(i) for i in model.terminators]
#     model.terminators_list = terminators_list
...
#     test_data = CustomImageTextMinicpmDatasetForRun(data, model, processor, max_length=max_input_tokens)
...
def parse_args():
    ...
    # --- 核心路径 ---
    parser.add_argument('--model_path', type=str, default='/data/huggingface/MiniCPM-V-2_6', help='基础模型的路径')
    parser.add_argument('--ckpt_path', type=str, default="ckpt/minicpm_mlp.pth", help='压缩器检查点文件的路径')
    ...
    parser.add_argument('--prefill_batch_size', type=int, default=1, help='[单次] Prefill批处理大小')
    parser.add_argument('--compress_batch_size', type=int, default=1, help='[单次] 压缩批处理大小')
    parser.add_argument('--decoding_batch_size', type=int, default=5, help='[单次] Decoding批处理大小')

    # --- 全局配置 ---
    parser.add_argument('--seed', type=int, default=48, help='随机种子')
    parser.add_argument('--torch_dtype', type=str, default='bfloat16', choices=['bfloat16', 'float16', 'float32'], help='PyTorch模型使用的数据类型')
...
def main():
    ...
    # --- 模式二：作为单次实验运行 ---
        ...
        experiments_to_run = [{
            ...
            "batching": [args.prefill_batch_size, args.compress_batch_size, args.decoding_batch_size]
        }]
    ...
    # 使用占位符加载模型和处理器
    model = AutoModel.from_pretrained(args.model_path, torch_dtype=torch_dtype, trust_remote_code=True).to(device)
    processor = AutoProcessor.from_pretrained(args.model_path, torch_dtype=torch_dtype, trust_remote_code=True)
    ...
    test_data = CustomImageTextMinicpmDatasetForRun(data, model, processor, max_length=args.max_input_tokens)
...
"""
python minicpm_testcon2.6.py --config configs/experiments.yaml 
python minicpm_testcon2.6.py --config configs/experiments.yaml --req_per_sec 5.0
python minicpm_testcon2.6.py --use_compression --comp_mode ExpectedAttention --prefill_batch_size 5 --compress_batch_size 5 --decoding_batch_size 5
python minicpm_testcon2.6.py --use_compression --comp_mode ccm --prefill_batch_size 15 --compress_batch_size 45 --decoding_batch_size 90

python minicpm_testcon2.6.py --use_compression --comp_mode ccm
python minicpm_testcon2.6.py --use_compression --comp_mode ExpectedAttention
